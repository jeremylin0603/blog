...
- 部分功能有多種表達方式，這邊認為沒必要都知道所以只會挑一個順眼的表達
...
- 單純的換行並不會真的換行，需要在換行的句尾加上兩個空白，才會被轉譯為`<br />`
...
- 可以有階層
- 內部一樣能用雙空白換行、標題、清單、城市區塊等其他 markdown 語法
...
- vuepress 會自動將外部連結添加 `target="_blank" rel="noopener noreferrer" 属性`，以避免外部連結取得不必要資訊，[詳細見此(但我還沒寫^^)]()
- 連結頁面錨點方法非原生 markdown，為 vuepress 提供的語法
...
- 使用組件需要另外配置設定[詳見](/blog/memo/vuepress#使用-vue-組件配置)
